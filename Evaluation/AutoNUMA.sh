...
# RPS_TEST_POOL=(0.5 5 10 12 13 14 15 16.3)
RPS_TEST_POOL=(15)
...
## Tiering-0.8(AutoNUMA=2)
DEMOTION_ENABLED=1
NUMA_BALANCING=2
WATERMARK_SCALE_FACTOR=100 # for (target free space size)=6 GB, Linux default=10
...
function enable_AutoNUMA(){
    # sudo swapon -a
    echo "$DEMOTION_ENABLED" | sudo tee /sys/kernel/mm/numa/demotion_enabled > /dev/null
    sudo sysctl -w kernel.numa_balancing=$NUMA_BALANCING > /dev/null
	# sudo sysctl -w vm.watermark_scale_factor=$WATERMARK_SCALE_FACTOR > /dev/null
	# echo 30 | sudo tee /proc/sys/kernel/numa_balancing_promote_rate_limit_MBps > /dev/null
	# echo 1 | sudo tee /proc/sys/kernel/numa_balancing_wake_up_kswapd_early > /dev/null
}
...
function run_vectordb() {
  ...
  echo "--------------------Before run_vectorDB------------------" > "$MIGRATION_LOG"
  sudo cat /proc/vmstat | grep numa >> "$MIGRATION_LOG"
  sudo cat /proc/vmstat | grep pro >> "$MIGRATION_LOG"
  sudo cat /proc/vmstat | grep demo >> "$MIGRATION_LOG"
...
#   sudo perf stat -C $VECTORDB_CPUS -e $STAT_EVENTS -d -o $OUT/stats.txt\
  ...
  echo "--------------------After run_question------------------" >> "$MIGRATION_LOG"
  sudo cat /proc/vmstat | grep numa >> "$MIGRATION_LOG"
  sudo cat /proc/vmstat | grep pro >> "$MIGRATION_LOG"
  sudo cat /proc/vmstat | grep demo >> "$MIGRATION_LOG"
}
